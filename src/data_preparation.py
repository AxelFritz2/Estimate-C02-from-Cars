...
class DataPreparation:
    ...
    def get_nan_table(self):
        nan_table = pd.DataFrame(columns=["Variable",
                                          "Pourcentages de valeurs manquantes", "Type"])
    ...
    def impute_train_test_numerical(self):
        ...
        self.train.loc[
            (self.train["Fuel_consumption_"].isna()) &
            (self.train["Ft"] == 'ELECTRIC'), "Fuel_consumption_"] = 0
        self.test.loc[
            (self.test["Fuel_consumption_"].isna()) &
            (self.test["Ft"] == 'ELECTRIC'), "Fuel_consumption_"] = 0

        mean_FC_by_Cn = self.train.groupby('Cn')[
            'Fuel_consumption_'].transform('mean')
        self.train['Fuel_consumption_'].fillna(mean_FC_by_Cn, inplace=True)
        self.train["Fuel_consumption_"].fillna(self.train[
            "Fuel_consumption_"].mean(), inplace=True)
